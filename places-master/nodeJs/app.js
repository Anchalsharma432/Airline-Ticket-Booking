...
const fs = require("fs"); // for reading and writing the users JSON file
...
app.post("/api/login", (req, res) => {
  ...
  fs.readFile(filePath, "utf-8", (err, data) => {
    ...
    try {
      ...
      return res
        .status(500)
        .send({ success: false, message: "Corrupted user data file." });
    }
...
app.get("/", (req, res) => {
  return res
    .status(404)
    .send({ success: false, message: "Successfully Deployed" });
});
...
app.get("/api/user/:email", (req, res) => {
  ...
  if (!fs.existsSync(filePath)) {
    return res
      .status(404)
      .send({ success: false, message: "User data not found." });
  }
...
app.put("/api/user/update", (req, res) => {
  ...
  fs.readFile("users.json", "utf-8", (err, data) => {
    if (err) {
      return res
        .status(500)
        .send({ success: false, message: "Error reading user data" });
    }
    ...
    fs.writeFile("users.json", JSON.stringify(users, null, 2), (writeErr) => {
      if (writeErr) {
        return res
          .status(500)
          .send({ success: false, message: "Error saving user data" });
      }
...
app.post("/api/reset-password", async (req, res) => {
  ...
  const users = readUsers();
  const user = users.find(
    (u) => u.resetToken === token && Date.now() < u.tokenExpiry
  );
...
const writeFlights = (data) =>
  fs.writeFileSync(flightsFile, JSON.stringify(data, null, 2));
...
app.put("/flights/:id", (req, res) => {
  ...
  res
    .status(200)
    .json({ message: "Flight updated successfully!", flight: flights[index] });
});
